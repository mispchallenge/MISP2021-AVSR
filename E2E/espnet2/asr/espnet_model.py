...
class ESPnetAVSRModel(AbsESPnetModel):  
    ...
    def encode(
        ...
        # print("---------video before frontend---------",file=log_file)
        # print(f"video_lengths:{video_lengths}",file=log_file)
        #video pre-encoder  shape of x is (B, T, 96, 96,3) -> (B,T,D)  25ps 
        video,video_lengths = self.video_frontend(video,video_lengths)
        B,video_T,video_D = video.size()
        ...
        # print("--------------after time align before encoder---------------",file=log_file)
        # print(f"video:{video.shape} length:{video_lengths},feat:{feats.shape},length{feats_lengths}",file=log_file)
    ...
    def forward(
        ...
        # log_file = open("/yrfs2/cv1/hangchen2/espnet/misp2021/asr1/expwavlip/test","a")
        # print("--------------original--------------",file=log_file)
        # print(f"video:{video.shape} length:{video_lengths},feat:{speech.shape},length{speech_lengths}",file=log_file)
        # print(f"text:{text.shape},length:{text_lengths}",file =log_file)
        # 1. Encoder
...
class ESPnetVSRModel(AbsESPnetModel):  
    ...
    def encode(
        ...
        #video pre-encoder  shape of x is (B, T, 96, 96,3) -> (B,T,D)  25ps 
        video,video_lengths = self.video_frontend(video,video_lengths)
        encoder_out, encoder_out_lens, _ = self.encoder(video,video_lengths)#[B,T,D]
        # print("--------------after encoder---------------",file=log_file)
        # print(f"hidden_shape:{encoder_out.shape} length:{encoder_out_lens}",file=log_file)
    ...
    def forward(
        ...
        # print(f"text:{text.shape},length:{text_lengths}",file =log_file)
        # 1. Encoder
        encoder_out, encoder_out_lens = self.encode(video,video_lengths)
        loss_att, acc_att, cer_att, wer_att = None, None, None, None
        loss_ctc, cer_ctc = None, None
        loss_transducer, cer_transducer, wer_transducer = None, None, None
        stats = dict()
