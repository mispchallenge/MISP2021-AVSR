...
def read_textgrid_from_file(filepath):
    ...
    # some of the textgrid have different format, if you have fix them just delete the filter_list
    filter_list = ["R12_S203204205_C09_I1_Near_203.TextGrid","R12_S203204205_C09_I1_Near_204.TextGrid","R12_S203204205_C09_I1_Near_205.TextGrid",'R53_S286287288289_C05_I1_Near_289.TextGrid', 'R53_S286287288289_C04_I1_Near_287.TextGrid', 'R53_S286287288289_C02_I2_Near_287.TextGrid', 'R53_S286287288289_C02_I2_Near_289.TextGrid', 'R61_S360361362363_C02_I2_Near_361.TextGrid', 'R53_S283284285_C02_I0_Near_284.TextGrid', 'R53_S283284285_C07_I0_Near_285.TextGrid', 'R53_S286287288289_C05_I1_Near_288.TextGrid', 'R53_S283284285_C02_I0_Near_285.TextGrid', 'R53_S283284285_C07_I0_Near_284.TextGrid', 'R53_S283284285_C02_I0_Near_283.TextGrid', 'R61_S360361362363_C04_I1_Near_363.TextGrid', 'R53_S286287288289_C02_I2_Near_286.TextGrid', 'R53_S286287288289_C02_I2_Near_288.TextGrid', 'R53_S286287288289_C01_I1_Near_288.TextGrid', 'R61_S360361362363_C02_I2_Near_363.TextGrid', 'R53_S286287288289_C01_I1_Near_289.TextGrid', 'R61_S360361362363_C04_I1_Near_362.TextGrid', 'R53_S286287288289_C01_I1_Near_287.TextGrid', 'R53_S283284285_C07_I0_Near_283.TextGrid', 'R61_S360361362363_C04_I1_Near_361.TextGrid', 'R61_S360361362363_C02_I2_Near_362.TextGrid', 'R53_S286287288289_C05_I1_Near_286.TextGrid', 'R53_S286287288289_C01_I1_Near_286.TextGrid', 'R53_S286287288289_C04_I1_Near_288.TextGrid', 'R61_S360361362363_C02_I2_Near_360.TextGrid', 'R61_S360361362363_C04_I1_Near_360.TextGrid', 'R53_S286287288289_C04_I1_Near_289.TextGrid', 'R53_S286287288289_C04_I1_Near_286.TextGrid', 'R53_S286287288289_C05_I1_Near_287.TextGrid'
    ,"R61_S360361362363_C03_I1_Near_360.TextGrid","R61_S360361362363_C03_I1_Near_361.TextGrid","R61_S360361362363_C03_I1_Near_362.TextGrid","R61_S360361362363_C03_I1_Near_363.TextGrid"]
    ...
    for tier_idx in range(size):
        if filename in filter_list:
            tiers.append(r12_read_tier_from_lines(tier_lines=lines[tier_start[tier_idx] + 1: tier_end[tier_idx]],filepath=filepath)) 
        else:
            tiers.append(read_tier_from_lines(tier_lines=lines[tier_start[tier_idx] + 1: tier_end[tier_idx]],filepath=filepath))

    return TextGrid(file_type=file_type, object_class=object_class, xmin=xmin, xmax=xmax, tiers_status=tiers_status,
                    tiers=tiers)


def read_tier_from_lines(tier_lines,filepath):
    ...
    assert len(tier_lines[5:]) == intervals_num * 5, print(intervals_num,filepath)
    # 'error lines'
...
def r12_read_tier_from_lines(tier_lines,filepath):
    assert 'class' in tier_lines[0], 'error line 0, {}'.format(tier_lines[0])
    tier_class = tier_lines[0].split('=')[1].replace(' ', '').replace('"', '').replace('\r', '').replace('\n', '')
    ...
    assert len(tier_lines[5:]) == intervals_num * 4, print(len(tier_lines[5:]),intervals_num,filepath,tier_lines[5:10],tier_lines[-3:])
    # 'error lines'

    intervals = []
    for intervals_idx in range(intervals_num):
        assert tier_lines[5 + 4 * intervals_idx + 0] == ' ' * 8 + 'intervals [{}]:\r\n'.format(intervals_idx + 1)
        # assert tier_lines[5 + 4 * intervals_idx + 1] == ' ' * 8 + 'intervals [{}]:\r\n'.format(intervals_idx + 1)
        intervals.append(read_interval_from_lines(
...
def prepare_mp4_scp(data_root, store_dir, set_type='train'):
    pos_dict = {"near":"near","far":"far","mid":"middle"}
    pos_id = pos_dict[store_dir.split("_")[-1]]
    if pos_id =="near":
        return None
    all_mp4_lines = []
    mp4_path_list = glob.glob(os.path.join(data_root, set_type, '*', '*{}*.mp4'.format(pos_id.title())))
...
def prepare_segments_text_utt2spk_worker(transcription_dir, set_type, store_dir, processing_id=None, processing_num=None,pos="far"):
    segments_lines = []
    text_sentence_lines = []
    utt2spk_lines = []
    tier_name = '内容层'
...
def prepare_segments_text_utt2spk_manager(transcription_dir, set_type, store_dir, processing_num=1,pos="far"):
    if processing_num > 1:
        ...
        segments_lines, text_sentence_lines, utt2spk_lines = prepare_segments_text_utt2spk_worker(
            transcription_dir=transcription_dir, set_type=set_type, store_dir=store_dir,pos=pos)
    # import pdb;pdb.set_trace()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--without_mp4', type=bool, default=False, help='don not prepare wav.scp')
    ...
    if "near" in args.store_dir or "gss" in args.store_dir:
        pos="near"
    else:
        pos="far"
    ...
    prepare_segments_text_utt2spk_manager(
        transcription_dir=args.transcription_dir, set_type=args.set_type, store_dir=args.store_dir, processing_num=args.nj,pos=pos)
