...
# video data preparation 
    ...
    _stage=0
    _roi_dir=/yrfs2/cv1/hangchen2/data/detection_results
    for pos in far ;do
        for setclass in  train eval ; do
            local/extract_far_video_roi.sh --nj 15 --stage ${_stage} ${setclass}_${pos} $_roi_dir/${setclass}/${pos} || exit 1;
        done 
    done
    fi
...
#video status
    if [ ${stage} -le 13 ] && [ ${stop_stage} -ge 13 ]; then
        ...
        #   ${train_cmd} JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
            ...
            #     --valid_data_path_and_name_and_type "${_asr_valid_dir}/roi.scp,video,video" \
            #     --valid_data_path_and_name_and_type "${_asr_valid_dir}/text,text,text" \
            #     --train_shape_file "${_logdir}/train.scp" \
            #     --valid_shape_file "${_logdir}/valid.scp" \
