...
expdir=expnear           # Directory to save experiments.
...
if ! "${skip_data_prep}"; then
    if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
        ...
        local/gssdata.sh --stage 0 ${local_data_opts}
    fi
...
# ========================== Data preparation is done here. ==========================
    ...
    if [ ${stage} -le 12 ] && [ ${stop_stage} -ge 12 ]; then
        log "video data preparation"
        _stage=0
        for set in $train_set $valid_set $test_sets ; do
              setclass=$(echo $set | awk '{split($1, arr, "_"); print arr[1]"_"arr[2]}')
              lippos=$(echo $set | awk '{split($1, arr, "_"); print arr[4]}')
              wavpos=$(echo $set | awk '{split($1, arr, "_"); print arr[3]}')
              local/extract_mid_lip_roi.sh --nj 15 --stage ${_stage} ${setclass} $lippos $wavpos  || exit 1; #far lip +  far wav 
        done  
    fi 

    if [ -n "${speed_perturb_factors}" ]; then 
        avsr_stats_dir="${expdir}/avsr_stats_raw_zh_char_sp"
    else
        avsr_stats_dir="${expdir}/nosp_avsr_stats_raw_zh_char"
    fi

    #collect audio and video stats
    if [ ${stage} -le 13 ] && [ ${stop_stage} -ge 13 ]; then
      
        _asr_train_dir="${data_feats}/${train_set}"
        _asr_valid_dir="${data_feats}/${valid_set}"
        log "Stage 13: AVSR collect audio and video stats: train_set=${_asr_train_dir}, valid_set=${_asr_valid_dir}"
        ...
        # 2. Generate run.sh
        log "Generate '${avsr_stats_dir}/run.sh'. You can resume the process from stage 13 using this script"
        ...
        ${train_cmd} JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
            ${python} -m espnet2.bin.avsr_train \
                ...
                --train_data_path_and_name_and_type "${_asr_train_dir}/roi.scp,video,v_no_interpolation" \
                --train_data_path_and_name_and_type "${_asr_train_dir}/text,text,text" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/${_scp},speech,${_type}" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/roi.scp,video,v_no_interpolation" \
        ...
        # Append the num-tokens at the last dimensions. This is used for batch-bins count
        <"${avsr_stats_dir}/train/text_shape" \
            awk -v N="$(<${token_list} wc -l)" '{ print $0 "," N }' \
            >"${avsr_stats_dir}/train/text_shape.${token_type}"

        <"${avsr_stats_dir}/valid/text_shape" \
            awk -v N="$(<${token_list} wc -l)" '{ print $0 "," N }' \
            >"${avsr_stats_dir}/valid/text_shape.${token_type}"
    fi
    ...
    if [ ${stage} -le 14 ] && [ ${stop_stage} -ge 14 ]; then
        _asr_train_dir="${data_feats}/${train_set}"
        _asr_valid_dir="${data_feats}/${valid_set}"
        log "Stage 14: AVSR Training: train_set=${_asr_train_dir}, valid_set=${_asr_valid_dir}"
        ...
        if [ "${feats_normalize}" = global_mvn ]; then
            ...
            _opts+="--normalize=global_mvn --normalize_conf stats_file=${avsr_stats_dir}/train/feats_stats.npz "
        fi
        ...
        if [ "${num_splits_asr}" -gt 1 ]; then
            ...
            _split_dir="${avsr_stats_dir}/splits${num_splits_asr}"
            if [ ! -f "${_split_dir}/.done" ]; then
                rm -f "${_split_dir}/.done"
                ${python} -m espnet2.bin.split_scps \
                    --scps \
                        "${_asr_train_dir}/${_scp}" \
                        "${_asr_train_dir}/text" \
                        "${_asr_train_dir}/roi.scp" \
                        "${avsr_stats_dir}/video_shape" \
            ...
            _opts+="--train_data_path_and_name_and_type ${_split_dir}/${_scp},speech,${_type} "
             _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/roi.scp,video,v_no_interpolation "
            _opts+="--train_data_path_and_name_and_type ${_split_dir}/text,text,text "
            _opts+="--train_shape_file ${_split_dir}/speech_shape "
            _opts+="--train_shape_file ${_split_dir}/train/video_shape "
            ...
            _opts+="--train_shape_file ${avsr_stats_dir}/train/video_shape "
            _opts+="--train_shape_file ${avsr_stats_dir}/train/text_shape.${token_type} "
        fi

        log "Generate '${avsr_exp}/run.sh'. You can resume the process from stage 14 using this script"
        ...
        ${python} -m espnet2.bin.launch \
            ...
            ${python} -m espnet2.bin.avsr_train \
                ...
                --valid_shape_file "${avsr_stats_dir}/valid/video_shape" \
                --valid_shape_file "${avsr_stats_dir}/valid/text_shape.${token_type}" \
                --resume true \
...
if ! "${skip_eval}"; then
    if [ ${stage} -le 15 ] && [ ${stop_stage} -ge 15 ]; then
        log "Stage 15: Decoding: training_dir=${avsr_exp}"
        use_wavaug_preprocessor=false
        ...
        mkdir -p "${avsr_exp}/${inference_tag}"; echo "${run_args} --stage 15 \"\$@\"; exit \$?" > "${avsr_exp}/${inference_tag}/run.sh"; chmod +x "${avsr_exp}/${inference_tag}/run.sh"
        if "${use_k2}"; then
        ...
        for dset in ${test_sets}; do
            ...
            ${_cmd} --gpu "${_ngpu}" JOB=1:"${_nj}" "${_logdir}"/asr_inference.JOB.log \
                ${python} -m ${asr_inference_tool} \
                    ...
                    --data_path_and_name_and_type "${_data}/roi.scp,video,v_no_interpolation" \
