...
# video data preparation 
    if [ ${stage} -le 12 ] && [ ${stop_stage} -ge 12 ]; then
        ...
        _stage=0
        # 1.created audio dir 2.create roi.scp for roilist just use lip roi which has been extract by chenhang 2.fix audio and video dir make sure they have same uttids
        for setclass in train dev eval ; do
          if [[ $train_set == *"far"* ]];then 
              local/extract_mid_lip_roi.sh --nj 15 --stage ${_stage} ${setclass}  middle far || exit 1; #middle lip +  far wav 
         else  
             local/extract_mid_lip_roi.sh --nj 15 --stage ${_stage} ${setclass}  middle near || exit 1; #middle lip +  near wav 
          fi 
        done  
    fi 
    ...
    if [ ${stage} -le 13 ] && [ ${stop_stage} -ge 13 ]; then
        ...
        _asr_train_dir="${data_feats}/${train_set}"
        _asr_valid_dir="${data_feats}/${valid_set}"
        log "Stage 13: AVSR collect video stats: train_set=${_asr_train_dir}, valid_set=${_asr_valid_dir}"
...
#avsr training 
    if [ ${stage} -le 14 ] && [ ${stop_stage} -ge 14 ]; then
        ...
        if [ "${num_splits_asr}" -gt 1 ]; then
            ...
            if [ ! -f "${_split_dir}/.done" ]; then
                ...
                ${python} -m espnet2.bin.split_scps \
                    --scps \
                        ...
                        "${avsr_stats_dir}/original_video_shape" \
                        "${avsr_stats_dir}/train/text_shape.${token_type}" \
                    --num_splits "${num_splits_asr}" \
                    --output_dir "${_split_dir}"
                touch "${_split_dir}/.done"
            ...
            _opts+="--train_data_path_and_name_and_type ${_split_dir}/text,text,text "
            _opts+="--train_shape_file ${_split_dir}/speech_shape "
            _opts+="--train_shape_file ${_split_dir}/train/original_video_shape "
            _opts+="--train_shape_file ${_split_dir}/text_shape.${token_type} "
            _opts+="--multiple_iterator true "
            ...
            _opts+="--train_shape_file ${avsr_stats_dir}/train/original_video_shape "
        ...
        ${python} -m espnet2.bin.launch \
            ...
            ${python} -m espnet2.bin.avsr_train \
                ...
                --valid_shape_file "${avsr_stats_dir}/valid/original_video_shape" \
