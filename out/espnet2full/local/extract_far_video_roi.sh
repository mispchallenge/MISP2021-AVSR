...
video_dir=${save_path}/"$audio_set"_video
...
if [ $stage -le 1 ]; then
  ...
  if [[ ! -f $video_dir/file.done ]]; then
    ...
    cat data/$audio_set/mp4.scp > $video_dir/mp4.scp 
...
if [ $stage -le 3 ]; then
  ...
  #add roi.sp
  if [ $audio_set == "train_far" ];then
    tag_set=train_far_sp
    echo "cat  $video_dir/roi.scp | sort -k 1 | uniq > dump/raw/$tag_set/roi.scp"
    ...
    echo "mv  dump/raw/$tag_set/roi.scp.sum  dump/raw/$tag_set/roi.scp"
