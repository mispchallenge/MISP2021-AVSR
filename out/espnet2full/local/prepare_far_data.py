...
def prepare_wav_scp(data_root, store_dir, set_type='train'):#set_type train dev eval  sub_dir:far mid near
    ...
    if pos_id in ["far","middle"] and set_type in ["train","dev"]:
        sub_dir = pos_id+'_wpe_beamformit' 
    else:
        sub_dir = pos_id
    if pos_id == "far" and set_type=="eval":
        sub_dir = pos_id+'_wpe_beamformit'
    all_wav_lines = []
    # print(f"here,data_root:{os.path.join(data_root,set_type,sub_dir)},store_dir:{store_dir}")
    wav_path_list = glob.glob(os.path.join(data_root,set_type,sub_dir, '*wav'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--only_mp4', type=bool, default=False, help='only prepare wav.scp')
