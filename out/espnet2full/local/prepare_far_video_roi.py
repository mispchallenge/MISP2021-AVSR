...
def segment_roi_json(roi_json_path, segments_name, segments_speaker, segments_start, segments_end, total_frames_num):
    with codecs.open(roi_json_path, 'r') as handle:
        if "Middle" in roi_json_path:
            roi_dic = json.load(handle)["frames"]
        else:
...
def segment_video_roi_json(video_path, roi_json_path, roi_store_dir, segments_name, segments_speaker, segments_start,
    ...
    #segments_roi_bound key->uid value list of list len[outlist]=frame_utterance_num len[innerlist]=4 x1 y1 x2 y2
    #frame2segment_roi_bound key->global frame index in mp4 which contain speak 
    # value-> [utter_id,relative index of frame in mp4,frame_len,bound]
    ...
    if frame2segment_roi_bound:
        ...
        #frame2segment_roi_bound key->frames_idx  value->frame_info_list
...
if __name__ == '__main__':
    ...
    parser.add_argument('data_root', type=str, default='dump/raw/org/train_far_video',
                        help='root directory of dataset')
    parser.add_argument('roi_json_dir', type=str, default='/yrfs2/cv1/hangchen2/data/detection_results',
                        help='directory of roi json')
    parser.add_argument('roi_store_dir', type=str, default='dump/raw/org/train_far_video/data',
    ...
    handle = codecs.open(os.path.join(args.data_root, 'log', 'roi.{}.scp'.format(ji+1)), 'w')
    for sentence_idx, sentence_path in enumerate(tqdm(all_sentences)):
