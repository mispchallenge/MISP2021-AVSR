...
if [ $stage -le 2 ] && [ $stop_stage -ge 2 ]; then
  ...
  python local/prepare_gss_data.py -nj 1 \
    --without_mp4 True \
    /raw7/cv1/hangchen2/misp2021_avsr/feature/misp2021_avsr/addition_far_audio_gss/wav \
    /raw7/cv1/hangchen2/misp2021_avsr/released_data/misp2021_avsr/addition_middle_video \
    /raw7/cv1/hangchen2/misp2021_avsr/released_data/misp2021_avsr/addition_near_transcription/TextGrid \
    eval\
    data/gss_addition_far

  local/prepare_gss_data.sh  --stage 2 ${data_dir} $enhancement_dir/${setclass}_${pos}_audio_gss \
            $setclass data/gss_addition_far || exit 1; #only use $4
fi


if [ $stage -le 3 ] && [ $stop_stage -ge 3 ]; then
  data_dir1=data/gss_eval_far
  data_dir2=data/gss_addition_far
  utils/combine_data.sh data/gss_sum_eval_far $data_dir1 $data_dir2
fi