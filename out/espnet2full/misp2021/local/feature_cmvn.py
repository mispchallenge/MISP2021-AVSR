...
def lps_cmvn(annotate, n_fft=400, hop_length=160, cmvn=None, **other_params):
    ...
    checkout_data_loader, _ = get_data_loader(
        annotate=annotate, items=['far_gss_wave'], batch_size=96, max_batch_size=512, repeat=1, max_duration=100, pad_value=[0],
...
if __name__ == '__main__':
    ...
    if 0 in stage:
        annotate = ['feature/misp2021_avsr/train_with_gss.json']
        ...
        safe_store(file='feature/misp2021_avsr/cmvn_wave/cmvn_fbank_htk_far_gss.pt',
        ...
        start_time = time.time()
        cmvn_output = fbank_cmvn(
            annotate=annotate, n_fft=400, hop_length=160, cmvn='feature/misp2021_avsr/cmvn_wave/cmvn_fbank_htk_far_gss.pt', f_min=0,
            f_max=8000, n_mels=40, sample_rate=16000, norm='slaney', preemphasis_coefficient=0.97, vtln=False,
            vtln_low=0, vtln_high=8000, vtln_warp_factor=1.)
        print('frames: {} time: {}s'.format(cmvn_output[0], time.time() - start_time))
