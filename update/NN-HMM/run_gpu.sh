...
train_list=()
predict_list=(0_1 0_2 1_3 1_4 1_5 1_6 1_9 0_4)
...
if [ $step == 0 ]; then
  ...
  ${python_path}python -m torch.distributed.launch --nproc_per_node=$gpu_num --master_port=12346 \
    local/run_gpu.py -c $step -m train -rs 123456 -be 0 -es 100 -sp 1 -pf 500 -ss train dev eval_addition \
    -ms ce acc -si 2 -ci 1 -co max -pd dev eval_addition -um -1
elif [[ "${predict_list[@]}" =~ "${step}" ]]; then
  CUDA_VISIBLE_DEVICES=$agi \
  ${python_path}python -m torch.distributed.launch --nproc_per_node=$gpu_num --master_port=12346 \
    local/run_gpu.py -c $step -m predict -rs 123456 -be 0 -es 100 -sp 1 -pf 500 -ss train dev eval addition \
    -ms ce acc -si 2 -ci 1 -co max -pd eval addition -um -1
fi
