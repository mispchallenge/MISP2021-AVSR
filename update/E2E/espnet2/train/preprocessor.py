...
class CommonPreprocessor(AbsPreprocessor):
    ...
    def _speech_process(
    ...
    ) -> Dict[str, Union[str, np.ndarray]]: #T,N(T)->T,N(T,1)
    
        assert check_argument_types()
        if self.speech_name in data:
            if self.train and (self.rirs is not None or self.noises is not None):
                speech = data[self.speech_name]
                nsamples = len(speech) #T

                # speech: (Nmic, Time)
                if speech.ndim == 1:
                    speech = speech[None, :] #1,T
                else:
                    speech = speech.T #1,T
        ...
        # assert 1==2,f"{data[self.speech_name].shape}"
        return data
...
class WavAugPreprocessor(AbsPreprocessor):
    def __init__(
        self,
        train: bool,
        token_type: str = None,
    ...
    def _speech_process(
        ...
        if self.speech_name in data:
            if self.train:
                speech = data[self.speech_name]
                # speech: (, Time)
                assert speech.ndim == 1 ,f" check {uid} it's a mutiplechannel-wav"
                speech = self.wavauger(speech)    #[T]->#[1,T]
                speech = speech.squeeze() ##[1,T]->[T]
                # assert 1==2,f"{speech.shape}"
                data[self.speech_name] = speech
